...
using Bicep.Types.AzureDevOps.Models;

namespace Bicep.Types.AzureDevOps;
...
public static class TypeGenerator
{
    ...
    public static TypeBase GenerateForRecord(TypeFactory factory, ConcurrentDictionary<Type, TypeBase> typeCache, Type type)
    {
        ...
        foreach (var property in type.GetProperties(BindingFlags.Public | BindingFlags.Instance))
        {
                ...
                propertyType.GetGenericArguments()[0] is { IsEnum: true } enumType)
            {
                ...
                propertyType.GetGenericTypeDefinition() == typeof(Nullable<>) &&
                propertyType.GetGenericArguments()[0] is { } nullableType)
            {
                typeReference = typeCache.GetOrAdd(nullableType, _ => factory.Create(() => GenerateForRecord(factory, typeCache, nullableType)));
            }
            else
            {
                throw new NotImplementedException($"Property {property.Name} has unsupported type {propertyType}");
            }
        ...
        return new ObjectType(type.Name, typeProperties, null);
    }
    ...
    public static Dictionary<string, string> GenerateTypes()
    {
        ...
        {
            ["personalAccessToken"] = new ObjectTypeProperty(factory.GetReference(secureStringType), ObjectTypePropertyFlags.Required, "Azure DevOps Personal Access Token"),
        }, null));
        ...
        var resourceTypes = new[] {
            GenerateResource(factory, typeCache, typeof(ServiceConnection)),
        };
